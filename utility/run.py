...
def main():
    ...
    test_ratio = float(cfg.get("test_ratio", 0.2))
    granularity = int(cfg.get("granularity", 10))
    filtering = bool(cfg.get("filtering", False))
    ...
    metrics_df.insert(5, "seed", seed)
    metrics_df.insert(6, "test_ratio", test_ratio)
    metrics_df.insert(7, "filtering", filtering)

    out_path = outdir / f"{dataset}__{method}__{mode}__run{run}__{sample}__seed{seed}__filtering{filtering}.csv"
    metrics_df.to_csv(out_path, index=False)
    print(f"Saved: {out_path.resolve()}")
    print(metrics_df)
